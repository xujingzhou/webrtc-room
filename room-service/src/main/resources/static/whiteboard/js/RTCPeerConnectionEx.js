    ...
    function SocketConnection(connection, connectCallback) {
        ...
        if (connection.enableLogs) {
            if (connection.socketURL == '/') {
                ...
                console.info('socket.io url is: ', connection.socketURL + parameters);
            }
        }
        ...
        try {
            connection.socket = io.connect(connection.socketURL, opts);
        } catch (e) {
            connection.socket = io(connection.socketURL, opts);
        }

        // socket本地变量
        connection.socket.userId = connection.userid;
        connection.socket.roomId = connection.sessionid;
        ...
        function onMessageEvent(data) {
            ...
            console.log("onMessageEvent -- data: " + data + ", connection.userid =" + connection.userid);

            var Obj = JSON.parse(data);
			var jsonObj = Obj.data.data;
			console.log("onMessageEvent - jsonObj = " + JSON.stringify(jsonObj));
			console.log("onMessageEvent - jsonObj.remoteUserId: " + jsonObj.remoteUserId + ", connection.userid: ", connection.userid);
			
            if (jsonObj.remoteUserId != connection.userid) {
                console.log("onMessageEvent - if (jsonObj.remoteUserId != connection.userid)");
                return;
            }

            // console.log("onMessageEvent - jsonObj.sender: " + JSON.stringify(jsonObj.sender));
            // console.log("onMessageEvent - connection.peers[jsonObj.sender]: " + JSON.stringify(connection.peers[jsonObj.sender]));
            // console.log("onMessageEvent - jsonObj.extra: " + JSON.stringify(jsonObj.extra) + ", jsonObj.message.extra: " + JSON.stringify(jsonObj.message.extra));
            if (connection.peers[jsonObj.sender] && connection.peers[jsonObj.sender].extra != jsonObj.message.extra) {
                connection.peers[jsonObj.sender].extra = jsonObj.extra;
                connection.onExtraDataUpdated({
                    userid: jsonObj.sender,
                    extra: jsonObj.extra
                });

                updateExtraBackup(jsonObj.sender, jsonObj.extra);

                console.log("onMessageEvent -- if (connection.peers[jsonObj.sender] && connection.peers[jsonObj.sender].extra != jsonObj.message.extra)");
            }

            if (jsonObj.message.streamSyncNeeded && connection.peers[jsonObj.sender]) {
                console.log("onMessageEvent -- jsonObj.message.streamSyncNeeded && connection.peers[jsonObj.sender]" );

                var stream = connection.streamEvents[jsonObj.message.streamid];
                if (!stream || !stream.stream) {
                    return;
                }

                var action = jsonObj.message.action;
                ...
                var type = jsonObj.message.type != 'both' ? jsonObj.message.type : null;
            ...
            if (jsonObj.message === 'dropPeerConnection') {
                connection.deletePeer(jsonObj.sender);
                return;
            }

            if (jsonObj.message.allParticipants && jsonObj.message.allParticipants.size > 0) {

                if (jsonObj.message.allParticipants.indexOf(jsonObj.sender) === -1) {
                    jsonObj.message.allParticipants.push(jsonObj.sender);
                    console.log("onMessageEvent -- if (jsonObj.message.allParticipants.indexOf(jsonObj.sender) === -1)" );
                }

                jsonObj.message.allParticipants.forEach(function(participant) {
                    console.log("onMessageEvent -- !connection.peers[participant]:" + !connection.peers[participant]);
            ...
            if (jsonObj.message.newParticipant) {
                console.log("onMessageEvent -- if (jsonObj.message.newParticipant)" );

                if (jsonObj.message.newParticipant == connection.userid) return;
                if (!!connection.peers[jsonObj.message.newParticipant]) return;

                mPeer.createNewPeer(jsonObj.message.newParticipant, jsonObj.message.userPreferences || {
                    localPeerSdpConstraints: {
                        OfferToReceiveAudio: connection.sdpConstraints.mandatory.OfferToReceiveAudio,
                        OfferToReceiveVideo: connection.sdpConstraints.mandatory.OfferToReceiveVideo
                    },
            ...
            if (jsonObj.message.readyForOffer) {
                console.log("onMessageEvent -- if (jsonObj.message.readyForOffer)" );
            ...
            if (jsonObj.message.newParticipationRequest && jsonObj.sender !== connection.userid) {
                console.log("onMessageEvent -- if (jsonObj.message.newParticipationRequest && jsonObj.sender !== connection.userid)" );

                if (connection.peers[jsonObj.sender]) {
                    connection.deletePeer(jsonObj.sender);
                }

                var userPreferences = {
                    extra: jsonObj.extra || {},
                    localPeerSdpConstraints: jsonObj.message.remotePeerSdpConstraints || {
                        OfferToReceiveAudio: connection.sdpConstraints.mandatory.OfferToReceiveAudio,
                        OfferToReceiveVideo: connection.sdpConstraints.mandatory.OfferToReceiveVideo
                    },
                    remotePeerSdpConstraints: jsonObj.message.localPeerSdpConstraints || {
                        OfferToReceiveAudio: connection.session.oneway ? !!connection.session.audio : connection.sdpConstraints.mandatory.OfferToReceiveAudio,
                        OfferToReceiveVideo: connection.session.oneway ? !!connection.session.video || !!connection.session.screen : connection.sdpConstraints.mandatory.OfferToReceiveVideo
                    },
                    isOneWay: typeof jsonObj.message.isOneWay !== 'undefined' ? jsonObj.message.isOneWay : !!connection.session.oneway || connection.direction === 'one-way',
                    isDataOnly: typeof jsonObj.message.isDataOnly !== 'undefined' ? jsonObj.message.isDataOnly : isData(connection.session),
                    dontGetRemoteStream: typeof jsonObj.message.isOneWay !== 'undefined' ? jsonObj.message.isOneWay : !!connection.session.oneway || connection.direction === 'one-way',
                    dontAttachLocalStream: !!jsonObj.message.dontGetRemoteStream,
                    connectionDescription: jsonObj,
                    successCallback: function() {}
                };

                connection.onNewParticipant(jsonObj.sender, userPreferences);
                return;
            }

            if (jsonObj.message.changedUUID) {
                if (connection.peers[jsonObj.message.oldUUID]) {
                    connection.peers[jsonObj.message.newUUID] = connection.peers[jsonObj.message.oldUUID];
                    delete connection.peers[jsonObj.message.oldUUID];
                }
            }

            if (jsonObj.message.userLeft) {
                mPeer.onUserLeft(jsonObj.sender);

                if (!!jsonObj.message.autoCloseEntireSession) {
                    connection.leave();
                }
            ...
            mPeer.addNegotiatedMessage(jsonObj.message, jsonObj.sender);
        }
        ...
        connection.socket.on('connect', function() {
            ...
            setTimeout(function() {
                var params = {"extra" : connection.extra};
                connection.socket.emit('extra-data-updated', JSON.stringify(params));
            }, 1000);
        ...
        connection.socket.on('user-disconnected', function(data) {
			console.log('user-disconnected: data = ' + data + ", connection.userid = " + connection.userid);
			var jsonObj = JSON.parse(data);
			var remoteUserId = jsonObj.data.remoteUserId;
		
			console.log('user-disconnected: remoteUserId = ' + remoteUserId);
        ...
        connection.socket.on('user-connected', function(data) {
			console.log('user-connected: data = ' + data + ", connection.userid = " + connection.userid);
			var jsonObj = JSON.parse(data);
			var userid = jsonObj.data.userid;
        ...
        connection.socket.on('userid-already-taken', function(data) {
			console.info('userid-already-taken: data = ' + data);
			var jsonObj = JSON.parse(data);
			console.info('userid-already-taken: useridAlreadyTaken = ' + jsonObj.data.useridAlreadyTaken + ", yourNewUserId = " + jsonObj.data.yourNewUserId);
            connection.onUserIdAlreadyTaken(jsonObj.data.useridAlreadyTaken, jsonObj.data.yourNewUserId);
        });
        ...
        connection.socket.on('number-of-broadcast-viewers-updated', function(data) {
            console.debug('number-of-broadcast-viewers-updated', data);
            connection.onNumberOfBroadcastViewersUpdated(data);
        });

        connection.socket.on('set-isInitiator-true', function(data) {
			console.info('set-isInitiator-true: data = ' + data + ", connection.sessionid = " + connection.sessionid);
			var jsonObj = JSON.parse(data);
    ...
    function MultiPeers(connection) {
            ...
            return {
                ...
                onPeerStateChanged: function(states) {
                    ...
                    if (states.iceConnectionState.search(/closed|failed/gi) !== -1) {
                        ...
                        connection.socket.emit('report-node-status', JSON.stringify(params), function() {});
                    }
                }
            };
        };
        ...
        this.addNegotiatedMessage = function(message, remoteUserId) {
            ...
            if (message.candidate) {
                ...
                if (connection.peers[remoteUserId]) {
                    // delete useless field. added by johnny xu,2021/6/22
                    var candi = {};
                    candi.candidate = message.candidate;
                    candi.sdpMid = message.sdpMid;
                    // candi.sdpMLineIndex = message.sdpMLineIndex;
                    candi["sdpMLineIndex"] = parseInt(message["sdpMLineIndex"]);
                    connection.peers[remoteUserId].addRemoteCandidate(candi);
...
//        }
        ...
        this.addRemoteSdp = function(remoteSdp, cb) {
            ...
            console.log("this.addRemoteSdp: " + JSON.stringify(remoteSdp));
    ...
    (function(connection) {
        ...
        mPeer.onNegotiationNeeded = function(message, remoteUserId, callback) {
            ...
            if (message.remoteUserId && message.message && message.sender) {
                ...
                console.log("mPeer.onNegotiationNeeded - if (message.remoteUserId && message.message && message.sender) ");
            }

            connectSocket(function() {
                connection.socket.emit(connection.socketMessageEvent, JSON.stringify(messageToDeliver), callback);
                console.log("mPeer.onNegotiationNeeded - connection.socket.emit: socketMessageEvent = " + connection.socketMessageEvent + " , messageToDeliver = " + JSON.stringify(messageToDeliver));
            });
        };
        ...
        mPeer.disconnectWith = function(remoteUserId, callback) {
            ...
            if (connection.socket) {
                connection.socket.emit('disconnect-with', JSON.stringify(messageToDeliver), callback || function() {});
            }
        ...
        function joinRoom(connectionDescription, cb) {
            ...
            connection.socket.emit('join-room', JSON.stringify(params), function(isRoomJoined, error) {
                if (isRoomJoined === true) {
                    ...
                    if (!!connection.peers[connection.sessionid]) {
                        console.log('!!connection.peers[connection.sessionid]');
                        // on socket disconnect & reconnect
                        return;
                    }

                    console.log("joinRoom - connectionDescription", JSON.stringify(connectionDescription));
        ...
        function openRoom(callback) {
            ...
            connection.socket.emit('open-room', JSON.stringify(params), function(isRoomOpened, error) {
        ...
        connection.changeUserId = function(newUserId, callback) {
            ...
            connection.socket.emit('changed-uuid', JSON.stringify(params), callback);
        };
...
//        }
        ...
        connection.socketURL = '/'; 
        connection.socketMessageEvent = 'msgEvent'; 
        connection.socketCustomEvent = 'socketCustomEvent'; 
        ...
        //     var params = {"socketCustomEvent" : connection.socketCustomEvent};
        //     connection.socket.emit('set-custom-socket-event-listener', JSON.stringify(params));
        // };

        connection.getNumberOfBroadcastViewers = function(broadcastId, callback) {
            if (!connection.socket || !broadcastId || !callback) return;

            var params = {"broadcastId" : broadcastId};
            connection.socket.emit('get-number-of-users-in-specific-broadcast', JSON.stringify(params), callback);
        };

        connection.onNumberOfBroadcastViewersUpdated = function(data) {
			console.log('onNumberOfBroadcastViewersUpdated: ', data);

			var jsonObj = JSON.parse(data);
			var broadcastId = jsonObj.data.broadcastId;
			var numberOfBroadcastViewers = jsonObj.data.numberOfBroadcastViewers;
		
            if (!connection.enableLogs || !connection.isInitiator) return;
            console.info('Number of broadcast (', broadcastId, ') viewers', numberOfBroadcastViewers);
        };
...
//        };
        ...
        connection.checkPresence = function(roomid, callback) {
            ...
            connection.socket.emit('check-presence', JSON.stringify(params), function(isRoomExist, _roomid, extra) {
                if (connection.enableLogs) {
...
//
//            if (typeof callback === 'function') {
//                var params = {"remoteUserId" : remoteUserId};
//                connection.socket.emit('get-remote-user-extra-data', JSON.stringify(params), function(extra, remoteUserId, error) {
...
//                connection.socket.emit('set-password', JSON.stringify(params), callback);
//            } else {
